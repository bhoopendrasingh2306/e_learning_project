  ...
  try {
    ...
    const admin_id = parseInt(Math.random() * 100000);
     ...
     if (!validator.isEmail(email)) {
      ...
      const adminCheck = await db.query(`select * from super_admin where email = '${email}'`);
    if(adminCheck.rowCount>0){
      res.status(400).json({result: "Admin with this Email Already exists"});
    }else{
      const response =
...
router.post("/login", async (req, res) => {
  ...
  const user = await db.query(`select * from super_admin where email = '${email}'`);
  if (user.rowCount == 0) {
    return res.status(400).json({result:"cannot find admin with this email"});
  }
  try {
